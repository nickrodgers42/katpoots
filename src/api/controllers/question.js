...
module.exports = function(server) {
  ...
  server.get("/api/questions/:quizId", getQuestions);
};

async function getQuestions(req, res, next) {
  try {
    const models = await loadModels();
    const questions = await models.question.find({ parent: req.params.quizId });
    res.json(questions);
    next();
  } catch (e) {
...
async function getQuestion(req, res, next) {
  try {
    const models = await loadModels();
    const question = await models.question.findById(req.params.questionId).populate({
      path: "answers"
    });
    if (!question) {
...
async function updateQuestion(req, res, next) {
  try {
    const models = await loadModels();
    const question = await models.question.findOneAndUpdate({ _id: req.params.questionId }, req.body, { new: true });
    if (!question) {
      res.status(404).send({ error: `Question with ID ${req.params.questionId} not found!` });
      return next();
    }
